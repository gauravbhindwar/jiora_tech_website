...
function GrowthHeader({ level }: { level: number }) {
  ...
  return (
    <div className="flex h-full min-h-[6rem] w-full items-end justify-center gap-1.5 rounded-xl border border-[var(--line)] bg-[var(--bg)] p-4">
      <div className="flex h-full w-full items-end justify-center gap-1.5">
        {heights.map((h, i) => (
          <div key={i} className="w-6 rounded-sm bg-[var(--fg)]" style={{ height: `${h * 28}px`, opacity: 0.4 + i * 0.18 }} />
        ))}
      </div>
      <span className="absolute bottom-2 right-3 font-mono text-[9px] tracking-widest text-[var(--muted)]">LEVEL {level + 1}</span>
    </div>
  );
}
...
export default function WhoWeWorkWith() {
  return (
    <section id="clients" className="bg-[var(--bg)] py-16">
      <div className="mx-auto max-w-6xl px-6">
        <div className="mx-auto max-w-2xl text-center">
          <p className="text-xs tracking-[0.2em] text-[var(--muted)]">WHO WE WORK WITH</p>
          ...
          <p className="mt-3 text-sm text-[var(--muted)]">Four growth stages — same logical bars, rising height = rising scale. No text cut.</p>
        </div>
